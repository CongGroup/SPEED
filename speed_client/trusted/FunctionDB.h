...
#define COPY_POINTER(pointer, pin, size) ::memcpy((pointer), (byte*)(pin), (size));\
(pointer) = (byte*)(pointer) + (size);

#define READ_OBJECT(pointer, obj) ::memcpy(&(obj), (pointer), sizeof(obj));\
(pointer) += sizeof(obj);

#define READ_POINTER(pointer, pout, size)  ::memcpy((pout), (byte*)(pointer), (size));\
(pointer) = (byte*)(pointer) + (size);
...
//TODO use output buffer data to build return object
//     and set other status, this can be done in a function like 
//     ReturnType build_ReturnType_for_FunctionName(byte* outputBuffer)
//     this func can cover gloab value change
if(doDedup)
{
	pb = output_buffer;
